#!/usr/bin/env python3
"""
Plot GRPO Checkpoint Training Progress on Icelandic EuroEval Validation Set

Creates visualizations showing model performance across training iterations.
Supports multiple training runs/variants.
...
Usage:
    python scripts/plot_icelandic_progress.py --input results/icelandic/icelandic_checkpoint_progress.json
"""
...
# ==================== Configuration ====================
DEFAULT_INPUT_JSON = "/home/x_anbue/euroeval-test/results/icelandic/icelandic_checkpoint_progress.json"
DEFAULT_OUTPUT_DIR = "/home/x_anbue/euroeval-test/results/icelandic/plots"

# Icelandic datasets with colors and display names
ICELANDIC_DATASETS = {
    "hotter-and-colder-sentiment": {"display": "Sentiment (MCC)", "color": "#E63946", "short": "Sentiment"},
    "scala-is": {"display": "ScaLA-IS (Ling.)", "color": "#F4A261", "short": "ScaLA"},
    "mim-gold-ner": {"display": "MIM-GOLD (NER)", "color": "#E9C46A", "short": "NER"},
    "nqii": {"display": "NQiI (Reading)", "color": "#2A9D8F", "short": "NQiI"},
    "icelandic-knowledge": {"display": "IS-Knowledge", "color": "#7209B7", "short": "Knowledge"},
    "winogrande-is": {"display": "WinoGrande-IS", "color": "#3A86FF", "short": "Winogrande"},
}
...
def create_learning_curve(
    ...
    ax.set_ylabel('Aggregate EuroEval Score (Icelandic)', fontweight='bold', fontsize=12)
    ax.set_title('GRPO Training Progress: Icelandic Validation Performance', 
    ...
    output_file = output_dir / 'icelandic_learning_curve.png'
...
def create_dataset_curves(
    ...
    for run_idx, (run_id, run_data) in enumerate(runs.items()):
        ...
        # Plot each dataset
        for dataset, ds_config in ICELANDIC_DATASETS.items():
            scores = []
            for d in data:
    ...
    ax.set_title('GRPO Training Progress: Per-Dataset Performance (Icelandic)', 
                 fontsize=16, fontweight='bold', pad=15)
    ...
    output_file = output_dir / 'icelandic_dataset_curves.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close()
    print(f"Saved: {output_file}")
...
def create_improvement_chart(
    ...
    ax.set_title('Score Improvement During GRPO Icelandic Training', 
                 fontsize=16, fontweight='bold', pad=15)
    ...
    output_file = output_dir / 'icelandic_improvement.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close()
    print(f"Saved: {output_file}")
...
def create_heatmap_timeline(
    ...
    iterations = [d["iteration"] for d in data]
    datasets = list(ICELANDIC_DATASETS.keys())
    ...
    # Labels
    dataset_labels = [ICELANDIC_DATASETS[d]['short'] for d in datasets]
    ...
    plt.title('Icelandic Benchmark Scores Across Training', 
              fontsize=16, fontweight='bold', pad=15)
    
    plt.tight_layout()
    
    output_file = output_dir / 'icelandic_heatmap.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close()
    print(f"Saved: {output_file}")
...
def create_run_comparison_radar(
    ...
    labels = [ICELANDIC_DATASETS[d]['short'] for d in datasets]
    ax.set_xticklabels(labels, fontsize=11, fontweight='bold')
    
    ax.legend(loc='upper right', bbox_to_anchor=(1.35, 1.1), fontsize=10, 
              frameon=True, facecolor='white', edgecolor='gray')
    
    plt.title('Run Comparison: Best Icelandic Checkpoints', 
              fontsize=16, fontweight='bold', pad=20)
    
    output_file = output_dir / 'icelandic_radar.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close()
    print(f"Saved: {output_file}")
...
def create_summary_stats(
    ...
    summary = f"""GRPO Icelandic Checkpoint Training Progress Summary
{'=' * 60}
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

Configuration:
  Language: {metadata.get('language', 'is')}
...
"""
    ...
    output_file = output_dir / 'icelandic_summary.txt'
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot GRPO Icelandic checkpoint training progress (supports multiple runs)"
    )
    parser.add_argument(
        "--input", "-i",
        type=str,
    ...
    print("=" * 60)
    print("GRPO Icelandic Checkpoint Progress Visualization")
    print("=" * 60)
